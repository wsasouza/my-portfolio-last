  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        ...
        if (!response.ok) throw new Error(data.error || 'Erro ao buscar projetos')
        ...
        >
          {projects.map((project) => (
              <Card as="li" key={project.id}>
                <div className="relative z-10 flex h-20 w-40 px-2 items-center justify-center rounded-lg bg-white shadow-md ring-1 shadow-zinc-800/5 ring-zinc-900/5 dark:border dark:border-zinc-700/50 dark:bg-zinc-800 dark:ring-0">
                  <Image
                    src={project.logo}
                    alt=""
